...
function App() {
  ...
  const [featuredReviews, setFeaturedReviews] = useState<(Review & { business: Business })[]>([]);
  ...
  const fetchFeaturedReviews = async () => {
    try {
      ...
      const reviewList = await Promise.all(reviewSnapshot.docs.map(async (doc) => {
        const reviewData = { id: doc.id, ...doc.data() } as Review;
        const businessDoc = await getDocs(query(collection(db, 'businesses'), where('id', '==', reviewData.businessId)));
        const businessData = businessDoc.docs[0].data() as Business;
        return { ...reviewData, business: businessData };
      }));
      setFeaturedReviews(reviewList);
    } catch (error) {
